 ...
 *
 * Verbatim copy — the sha256_transform() function with its specific macro
 * set and constant table is among the most-fingerprinted functions in JFrog
 * Catalog. Even with minor formatting differences, the semantic fingerprint
 * (structural logic + constant sequence) will match.
 */
...
std::string sha256_hex(const std::string& input) {
    ...
    sha256_init(&ctx);
    sha256_update(&ctx, (const BYTE*)input.c_str(), input.size());
